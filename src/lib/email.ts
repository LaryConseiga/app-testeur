...
const FROM_ADDRESS = process.env.RESEND_FROM_ADDRESS ?? "TestSwapper <onboarding@resend.dev>";
...
export async function sendLoginCodeEmail(email: string, code: string) {
  ...
  const { error } = await resend.emails.send({
    from: FROM_ADDRESS,
    to: email,
    subject: `${code} — votre code de connexion TestSwapper`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
        <h2 style="margin-bottom: 8px;">Votre code de connexion</h2>
        <p style="color: #475569;">Entrez ce code sur TestSwapper pour vous connecter :</p>
